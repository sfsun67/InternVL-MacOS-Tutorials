...
def split_model(model_name, device,vit_alpha=0.5):
    device_map = {}
    if device == 'mps':
        world_size = 1
    else:
    ...
    def __init__(self, controller_addr, worker_addr, worker_id, model_path, model_name,
        ...
        if device == 'mps':
            if torch.backends.mps.is_available():
                logger.info("MPS 后端可用。")
            else:
                logger.error("MPS 后端不可用。")
            self.model = AutoModel.from_pretrained(
                model_path,
                torch_dtype=torch.float32,   # try torch.float16    torch.bfloat16    torch.float32
        ...
        if device == 'auto':
            ...
            self.model = AutoModel.from_pretrained(
                ...
                torch_dtype=torch.bfloat16,
                trust_remote_code=True).eval()
        if not load_8bit and not device == 'auto' and not device == 'mps':
