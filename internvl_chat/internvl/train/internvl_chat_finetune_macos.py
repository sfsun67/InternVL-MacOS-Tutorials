...
# NOTE: 现在稳定需要先运行无参数 Debug，然后才能运行带参数 Debug. 直接运行参数 Debug 会报错。
...
# from internvl.dist_utils import init_dist   # OPTIMIZE 相关函数都禁用了
...
def main():
    ...
    def apply_necessary_patches():
        # Apply necessary patches for the transformers library
        # replace_llama_rmsnorm_with_fused_rmsnorm()    # NOTE 使用 Nvidia 库加速 RMSnorm 计算 # fix 将CPU运算转移到CPU运算上，所以注释这个函数应该没有影响
        # 这些函数用于替换默认的训练数据加载器和采样器，以适配特定的训练需求或优化运行效率。
        replace_train_sampler()    # 对数据进行重排列的，把长度差不多的数据放在一个 banch 里。
    ...
    def parse_input_arguments():
        ...
        # init_dist(launcher=launcher, backend='nccl')    # fix MacOS 只有一个16核显卡，不需要初始化 # init_dist 根据环境变量选择分布式启动器（如 slurm），并设置 NCCL 后端。
        parser = HfArgumentParser((ModelArguments, DataTrainingArguments, TrainingArguments))    # 通过 HfArgumentParser 从命令行或 JSON 文件中解析模型参数 (ModelArguments)、数据参数 (DataTrainingArguments) 和训练参数 (TrainingArguments)。

        #这段代码的目的是根据传递给脚本的参数，灵活地从 JSON 文件或命令行参数中获取配置，并将这些配置应用到模型训练过程中。
        ...
        # init wandb proj name
        if training_args.report_to[0] == 'wandb':
            import re
            filename = parser.prog
            wandb_project = re.sub(r'\.py$', '', filename)

        else:   
            wandb_project = None
        
        return model_args, data_args, training_args, wandb_project
    ...
    def load_pretrained_models(model_args, data_args, training_args):
        ...
        tcs_loader = TCSLoader('~/petreloss.conf') if has_tcs_loader else None    # tcs_loader 是一个用于加载视频帧的对象

        # IDEA 应该是禁用这里，不使用 flash attention。在原本的 internvl attention 中，也有默认的非 flash attention 实现
        if data_args.use_packed_ds:
            pass
            # replace_internlm2_attention_class()    # FIX 如何用普通的 attention 替换？想法，先把一系列链路禁用看看 tag：【replace_internlm2_attention_class()】
            # replace_qwen2_attention_class()
            # replace_phi3_attention_class()
            # replace_llama_attention_class()
        ...
        # 优先加载预训练的 InternVLChatModel： 	•	如果提供了路径，直接加载完整的多模态模型。
        if model_args.model_name_or_path is not None:
            ...
            if config.llm_config.model_type == 'internlm2':
                ...
                config.llm_config.attn_implementation = 'eager'  # for InternLM  eager
            ...
            # fix non flashattention
            config.vision_config.use_flash_attn = False
            
            model = InternVLChatModel.from_pretrained(
                # model_args.model_name_or_path, torch_dtype=torch.bfloat16, config=config)
                model_args.model_name_or_path, config=config)
        # 手动组合模型： 	•	如果没有完整路径，则分别加载视觉模型 (ViT) 和语言模型 (LLaMA) 并构造最终模型。
        else:
            ...
            if llm_config.model_type == 'internlm2':
                ...
                llm_config.attn_implementation = 'eager'  # for InternLM eager or compile
                logger.info('Using eager for InternLM')
            else:
                model_type = AutoModelForCausalLM
                llm_config._attn_implementation = 'eager'  # for LLaMA eager or compile
                logger.info('Using eager for LLaMA')
            llm = model_type.from_pretrained(   
                model_args.llm_path, torch_dtype=torch.bfloat16,
                config=llm_config, trust_remote_code=True)
            logger.info('Building InternVLChatConfig...')
            internvl_chat_config = InternVLChatConfig(      # IDEA 这是什么？如果想加一个模块，是不是从这里修改模型权重的？
        ...
        # img_context_token_id 的值被存储在 model 对象的 img_context_token_id 属性中。这通常用于在模型的前向传播或其他操作中引用这个特定的标识符。
        model.img_context_token_id = img_context_token_id

        # 这样的代码通常用于对齐模型配置和数据采样的下采样比例。具体来说，这行代码可能起以下几个作用：
        assert model.config.downsample_ratio == data_args.down_sample_ratio

        '''
        这段代码的目的是加载和配置一个多模态模型（包括视觉模型和语言模型），并准备训练数据集。
        '''
        # IDEA import 学习如何修改模型权重参数
        # 如果 model_args.mlp_path 不为空，则加载预训练的 MLP 投影器的状态字典，并将其加载到模型的 mlp1 层中。
        ...
        '''配置模型的视觉部分：
        获取视觉模型的补丁大小（patch_size）。
        打印模型和数据参数中的图像大小配置。
        如果模型配置中的图像大小与数据参数中的图像大小不一致，则调整位置嵌入（position embedding）的大小，并更新配置。
        计算并设置模型的图像标记数量（num_image_token）。'''
        ...
        # 调整语言模型的词汇表大小：如果有新的标记（num_new_tokens），则调整语言模型的词汇表大小，并初始化新标记的嵌入权重。
        if num_new_tokens > 0:
        ...
        # 配置模型的缓存和梯度检查点：
        model.language_model.config.use_cache = False    # 禁用语言模型的缓存。
        model.vision_model.gradient_checkpointing = True    # 启用视觉模型和其编码器的梯度检查点。
        model.vision_model.encoder.gradient_checkpointing = True    
        if model_args.grad_checkpoint:    # 如果 model_args.grad_checkpoint 为真，则启用语言模型的梯度检查点。
            model.language_model._set_gradient_checkpointing()
            
        return last_checkpoint, tokenizer, tcs_loader, model

    def dataset_preparation(tokenizer, tcs_loader, model):
        # 使用提供的参数构建训练数据集。
        ...
        # 数据整理函数（collator）在数据加载过程中起着关键作用，特别是在将多个样本打包成一个批次时。它负责将单个样本转换为模型可以处理的批次格式，通常包括将不同长度的序列填充到相同长度、将数据转换为张量等操作。
        if data_args.use_packed_ds:
            collator = partial(
                packed_collate_fn,
    ...
    def model_configuration():
        # IDEA 冻结参数。在训练自定义的模块时，很有用。
        # 定义冻结参数的辅助函数：定义一个辅助函数 _freeze_params，用于冻结模型模块的参数，使其在训练过程中不更新。
        def _freeze_params(module):
            for param in module.parameters():
                param.requires_grad = False

        #  模型参数调整
        # 冻结模型的部分参数（如视觉模型主干或语言模型头部）。
        ...
        if model_args.use_backbone_lora:    # 启用 LoRA（低秩适配器）以减少训练参数。
    ...
    # 训练前准备
    apply_necessary_patches()
    model_args, data_args, training_args, wandb_project = parse_input_arguments()
    setup_logging()
    last_checkpoint, tokenizer, tcs_loader, model = load_pretrained_models(model_args, data_args, training_args)
    train_dataset, collator = dataset_preparation(tokenizer, tcs_loader, model)
    model_configuration()
    
    # Initialize wandb
    wandb.init(project=f"{wandb_project}_20250129_234642_test", config=training_args.to_sanitized_dict())
    ...
    # Training # IDEA 训练主函数
    # 从指定的检查点继续训练，或者从头开始。  训练完成后保存模型和分词器。
